...
def test_create_variant_probe_set(Graph, kmer_size):
    ...
    assert len(variant_search.create_variant_probe_set("T1C").refs[0]) == (2 * kmer_size + 1)
...
def test_search_for_variant(Graph):
    ...
    variant_probe_set = variant_search.create_variant_probe_set("T1C")
...
def test_search_for_amino_acid_mutation():
    ...
    variant_search = BIGSIAminoAcidMutationSearch(bigsi, "bigsi/tests/data/ref.fasta", "bigsi/tests/data/ref.gb")

    var_name1 = variant_search.aa2dna.get_variant_names("rpoB", "S450X", True)[0]
    var_name2 = variant_search.aa2dna.get_variant_names("rpoB", "S450X", True)[4]
    ...
    bigsi.build([bloom1, bloom2, bloom3, bloom4], ["ref1", "alt1", "ref2", "alt2"])

    results = variant_search.search_for_amino_acid_variant("rpoB", "S", 450, "X")
    assert results.get("rpoB_S450X").get("ref1").get("genotype") == "0/0"
    assert results.get("rpoB_S450X").get("ref1").get("aa_mut")[:-1] == "S450"
    assert results.get("rpoB_S450X").get("ref1").get("variant")[:-3] == var_name1[:-3]

    assert results.get("rpoB_S450X").get("ref2").get("genotype") == "0/0"
    assert results.get("rpoB_S450X").get("ref2").get("aa_mut")[:-1] == "S450"
    assert results.get("rpoB_S450X").get("ref2").get("variant")[:-3] == var_name2[:-3]
