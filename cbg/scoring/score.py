...
class Scorer():
    ...
    def __init__(self, DB_SIZE, MATCH=1, MISMATCH=2, LAMBDA_UNGAPPED=1.330, K_UNGAPPED=0.621, LAMBDA_GAPPED=1.28, K_GAPPED=0.46):
    ...
    def calculate_score(self, score_counter, convert):
        ...
        return {"score": round(mean_score*convert, 2), "min_score": round(min_score*convert, 2), "max_score": round(max_score*convert, 2),
                "max_mismatches": math.ceil(max_total_N_snps), "min_mismatches": math.floor(min_total_N_snps), "mismatches": math.ceil(math.ceil(min_total_N_snps) + (0.05 * math.floor(max_total_N_snps)))}
